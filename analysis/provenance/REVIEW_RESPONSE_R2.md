...
# Round six: separating the corrected result from the history of the corrections

The manuscript is rebuilt as a standalone scientific study. The correction
history is not deleted -- it lives here and in the other provenance files -- but
it no longer supplies the manuscript's narrative voice. Nothing in the abstract,
introduction, results, discussion or conclusion now discusses a previous version
of this paper, and the words *defect*, *withdrawn*, *corrigendum*, *earlier
draft*, *working paper* and *as reported* do not appear in the body text.

## The audit trail table, retired from the body and preserved here

Table 3 of the previous version listed every correction under four headings. Its
first block is now Table 1 of the manuscript, restated as a neutral taxonomy of
**five implementation failure modes** with the diagnostic that identifies each
and the measured violation rate with the failure mode present and absent. The
other three blocks are retired from the body and recorded in full below, since
they are statements about the history of this project rather than about
tail-risk forecasting.

### Defects in the analysis code

| item | diagnostic | as reported | corrected |
|---|---|---|---|
| Sign inversion in both tail-closure scripts | reconstruction compared against the promoted series | "R > 1 under every closure" | R in [0.005, 1.70] |
| Sign inversion in the Z2 routine | second implementation written from the published definition | 24/24 assets pass at pi-hat = 0.39 | 0/24; Z2 = -144, -389 |
| Inner-seven-deciles check | fits a Student-t to points generated by a Student-t | "most demanding check" | cannot fail |

The Z2 sign convention survives in the supplement (S.1) as a *methodological*
caution rather than as an incident report: the statistic requires ES as a
positive magnitude, the stored column is a negative lower-tail quantity, and a
structural check on the forecast series would not have caught it because the
series were well-formed and the statistic was not. That is a genuine limit on
the paper's own thesis and is stated as one.

### Claims withdrawn or restated

| claim | diagnostic | as reported | corrected |
|---|---|---|---|
| Predictive interface drives tail failure | corrected panel ordering; the two Moirai releases differ on three axes | 1.5% vs 98.8% within family | 0.0154 vs 0.0178 |
| R as an audit statistic | Spearman(R-bar, pi-hat_raw) = 0.991 | "dominates both backtests" | a re-encoding of pi-hat |
| Gating rule cost | re-evaluated on the calibration window | half the damage, no upgrades lost | 25.3%, 20 upgrades lost |
| Share of miscalibration removable by a shift | Gneiting--Resin decomposition | 35% | 27.0% |
| "Signal-preserving vs effective replacement" partition | no cutoff distinguished by the data | two regimes on R > 1 | not restored; R reported as a continuous magnitude |

Each of these is now stated positively in the manuscript, without reference to
what it replaced. The interface claim became Section 5.2.2, which says only that
the two interfaces do not separate and that the tail-completion rule is part of
the forecaster. The R claim became Section 5.2.1, which presents R as a
scale-interpretable restatement of the violation rate and not as an independent
instrument. The gating cost is reported at its deployable value throughout, with
the oracle labelled as an upper bound.

### Repairs to the theory

| item | issue | as stated | corrected |
|---|---|---|---|
| Lemma S.9.1 | the forward sigma-field inclusion fails for an infinite context | beta_S(k) <= beta_r(k-1) | beta_S(k) <= beta_r(k-m) |
| Proposition S.9.4 | the regret bound does not imply the coverage statement | regret argument | exact telescoping |
| Proposition S.9.6 | drift measure not estimable from one path | stated as a bound | labelled heuristic |

All three repairs stand in the supplement. Their remarks are rewritten to state
what the mathematics requires rather than what an earlier version asserted; the
content, including the finite-context restriction and the EWMA approximation
argument, is unchanged.

## The section that was removed outright

"The uncomfortable retrospective" (6.4) argued that the instruments had flagged
every defect family in printed tables before any defect was known, and that the
failure was one of interpretation. The argument depends on reading superseded
tables as evidence, and it depends on the "signal-preserving vs effective
replacement" partition that the review asked not to restore. It is removed. Its
one transferable point -- that a large correction magnitude is a question about a
pipeline rather than a finding about a forecaster -- survives in Section 5.2.1
and in the practitioner guidance, stated directly.

## Two contradictions found during the sweep and fixed

1. **Supplement S.1 contradicted its own table.** The closing paragraph read
   "The Z2 statistic does not reject for any model in either the raw or corrected
   configuration", while Table S.2 shows 0/24 passing for both truncated Chronos
   series raw and 3/24 corrected, and 9/24 for raw Lag-Llama. The paragraph was
   stale from before the sign fix. Rewritten against the table: Z2 rejects the
   truncated series on all 24 assets raw and 21 of 24 corrected, and the pass
   counts rather than the means are the informative column.

2. **EWMA was documented two ways.** Section 3.3.3 said all classical benchmarks
   are estimated on rolling windows of 250 observations; Table B.1 (supplement
   S.8) records EWMA as `lambda = 0.94, Recursive`, and Lemma S.9.1's remark
   states that the series came from the RiskMetrics recursion. The manuscript now
   carries the exception in all three places where the convention is quoted
   (Sections 3.1.1 and 3.3.3, and the lemma remark), with the numerical argument
   that the truncation at m = 600 is indistinguishable from the recursion.
