...
\input{Quantlets/CO_identification/tab_identification.tex}
...
\input{Quantlets/CO_diagnostic_scale/tab_scale_diagnostic.tex}
...
interpreted. The sign of the slope $\hat b$ gives an independent detector for the regime
split. A negative fitted slope is a near-certain replacement signature:
$\hat b<0$ implies $R>1$ in $51$ of the $52$ pairs where it occurs, so an
inverted slope is a \emph{sufficient} condition for replacement rather than an
equivalent classifier. The disagreement runs one way only --- replacement
pairs ($R>1$) carrying a positive slope, which the sign test alone does not
flag --- and the two diagnostics agree on the regime for $78.7\%$ of the $216$
pairs overall (Table~\ref{tab:scale_regime_crosstab}). The scale diagnostic
therefore does more than register a caveat: it independently reproduces the
paper's replacement/signal-preserving split from an entirely different
estimator.

\input{Quantlets/CO_diagnostic_scale/tab_scale_regime_crosstab.tex}
...
this is the mechanism behind its Green-zone rate of $97.5\%$.%
\footnote{The head-to-head comparison of Table~\ref{tab:baselines_aci}
evaluates the rolling estimator on the $70/30$ test split with the Basel zone
scored on the final $250$-day window, over the $216$-pair subset, and reports
$94.9\%$ Green at the same $0.010$ mean violation rate; the $97.5\%$ headline is
the full-sample, full-history Basel scoring over all pairs. The two protocols
agree on coverage and differ only in the zone-counting window and the pair
set.} The gap between
...
\input{Quantlets/CO_aci_baseline/tab_baselines_aci.tex}

\input{Quantlets/CO_aci_baseline/tab_aci_gamma_sensitivity.tex}

\paragraph{Verdict.}
\input{Quantlets/CO_aci_baseline/verdict_aci.tex}
