...
# ── The appendix table ──────────────────────────────────────────────
# tab_gbm_tuned.tex had no producer: it was written by hand in May and still
# reported denominators of 9. It is emitted here from the grid this script
# computes, so the two cannot drift apart again.
def write_grid_table():
    import pandas as _pd
    summ = _pd.read_csv(OUT / 'tuned_gbm_qr_summary.csv')
    lines = ["% Generated by Quantlets/CO_baseline_comparison_tuned/run_tuned_gbm_qr.py",
             r"\begin{tabular}{@{}rrr rrrr@{}}", r"\hline\hline",
             r"$n_\text{est}$ & $d$ & $\eta$ & $\hat\pi$ & Kupiec rej.",
             r"& QS ($\times10^{4}$) & Green \% \\", r"\hline"]
    for _, r in summ.iterrows():
        pi = f"{r['pi_hat']:.3f}".lstrip('0')
        lines.append(f"{int(r['n_est'])} & {int(r['max_depth'])} & {r['lr']:.2f} "
                     f"& {pi} & {int(r['kup_rej'])}/{int(r['n_pairs'])} "
                     f"& {r['QS_x1e4']:.2f} & {r['green_pct']:.1f} \\\\")
    lines += [r"\hline\hline", r"\end{tabular}"]
    (OUT / 'tab_gbm_tuned.tex').write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Saved to {OUT / 'tab_gbm_tuned.tex'}")
