...
def write_tex(df):
    ...
    lines.append(r'The closure rule is not a sensitivity in the usual sense: it')
    lines.append(r'moves $R$ by a factor of 3 to 76 within a single model--asset pair')
    lines.append(r'and can change the Basel zone. The inner-7 variant reproduces the')
    lines.append(r'full Student-$t$ fit to six decimals because both fit a Student-$t$')
    # The shipped table's note ends "see the discussion", not "it cannot fail":
    # the wording was edited in the .tex after generation, so every rebuild
    # silently reverted it. The editorial choice belongs in the emitter.
    lines.append(r'to points generated by a Student-$t$; see the discussion.}')
