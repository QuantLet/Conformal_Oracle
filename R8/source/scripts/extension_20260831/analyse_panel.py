...
CLASSES={'Equity':['ASX200','BOVESPA','FCHI','FTSE100','GDAXI','HSI','ICLN','NIFTY','NIKKEI','SP500','STOXX'],
         'Bond ETF':['CBU0','IBGL','TLT'],'Commodity':['DJCI','GOLD','NATGAS','WTI'],
         'Crypto':['BTC','ETH'],'FX':['AUDUSD','EURUSD','GBPUSD','USDJPY']}
CLASS={a:c for c,assets in CLASSES.items() for a in assets}
...
def main():
    ...
    assert len(ledger)==216*4 and len(base)==216*10
    ...
    bootstrap(frames)
    print('Complete panel: 216 pairs, 864 tail-level cells, 288 master rows; sensitivity and paired intervals saved',flush=True)
