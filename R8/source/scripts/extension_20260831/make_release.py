...
import shutil
import zipfile
from paper_scope import ROOT as PAPER_ROOT
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output',type=Path,default=REPO/'release/R8_20260831');a=ap.parse_args()
    ...
    assert json.loads((PAPER_ROOT.parent/'selection_validation.json').read_text())['retained_pair_statistics_unchanged']
    ...
    for name in ['r8_decision','r8_mechanism','r8_integration','r8_external','r8_regime','r8_native_panel']:
        copy_tree(REPO/'research'/name,out/'research'/name)
    for name in ['r8_decision','r8_mechanism','r8_risk_integration','r8_external','r8_regime','r8_native_panel']:
    ...
    (out/'README.md').write_text((REPO/'research/r8_native_panel/README.md').read_text()+
    ...
    for name in ['IRFA_AUGUST2026_R8_VALIDATION.md','IRFA_R8_FIGURE_STYLE_VALIDATION.md',
                 ...
                 'IRFA_REVIEW_STATE.md','IRFA_NATIVE_PANEL.md']:
    ...
    (out.parent/'R8_release_receipt.json').write_text(json.dumps(result,indent=2)+'\n')
