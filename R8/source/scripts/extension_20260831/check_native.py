...
def main():
    ...
    root=inf.ROOT/'artifacts/extension_20260831';torch.set_num_threads(2)
    if a.model=='lagllama' and (root/'calendar_primary.json').exists():
        import subprocess,sys
        assert a.device=='mps','Actual-calendar replay uses the recorded MPS environment'
        subprocess.run([sys.executable,str(inf.ROOT/'research/r8_review/replay_primary_calendar.py')],check=True)
        return
