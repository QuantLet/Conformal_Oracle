...
def build(dest):
    ...
    for b,word in [(20,'Twenty'),(60,'Sixty')]:
        ...
        for col,tag in [('lower','Lo'),('upper','Hi')]:macro('nTenPotPoint'+word+tag,r[col])
    for method,tag in [('Raw','Raw'),('Shift-CP','Static')]:
        macro('nTen'+tag+'Ind',native.xs('Static' if method=='Shift-CP' else 'Raw',level='method').independence_rejections.sum(),0)
        assert native.xs('Static' if method=='Shift-CP' else 'Raw',level='method').independence_rejections.sum()==(pairs[(pairs.method==method)&(pairs.p_ind<.05)]).shape[0]
    ...
    lines=[r'\begingroup\setlength{\tabcolsep}{3.5pt}',r'\begin{tabular}{lrrrrrrr}',r'\toprule',r'Forecaster & QS raw & QS static & QS rolling & $\pi$ raw & $\pi$ static & UC & Ind. \\',r'\midrule']
    for model in MODELS:
        raw=native.loc[(model,'Raw')];stat=native.loc[(model,'Static')];roll=native.loc[(model,'Rolling250')]
        assert raw.independence_available==24 and stat.independence_available==24
        lines.append(f'{LABELS.get(model,model)} & {raw.QS_x10000:.4f} & {stat.QS_x10000:.4f} & {roll.QS_x10000:.4f} & {raw.pi_mean*100:.3f} & {stat.pi_mean*100:.3f} & {int(raw.kupiec_rejections)}/{int(stat.kupiec_rejections)} & {int(raw.independence_rejections)}/{int(stat.independence_rejections)}'+r' \\')
    lines += [r'\midrule',r'\multicolumn{8}{l}{\emph{Additional supervised benchmark, same asset dates}} \\']
    raw=gbm.loc['Raw'];stat=gbm.loc['Static'];roll=gbm.loc['Rolling250']
    gind=(lambda r:str(int(r.independence_rejections)) if 'independence_rejections' in gbm.columns else '--')
    lines.append(f'Direct-quantile GBM & {raw.QS_x10000:.4f} & {stat.QS_x10000:.4f} & {roll.QS_x10000:.4f} & {raw.pi_mean*100:.3f} & {stat.pi_mean*100:.3f} & {int(raw.kupiec_rejections)}/{int(stat.kupiec_rejections)} & {gind(raw)}/{gind(stat)}'+r' \\')
    lines += [r'\bottomrule',r'\end{tabular}',r'\endgroup'];write('tab_native_forecasters.tex','\n'.join(lines))
    selected=['Raw','Shift-CP','Vol-ERM','State-L1','POT-Shift','POT-Vol','Rolling500','Loss-gate','Past-minimum']
    lines=[r'\begingroup\setlength{\tabcolsep}{3pt}',r'\begin{tabular}{lrrrrll}',r'\toprule',r'Method & QS & Viol. (\%) & UC & Worse & Pointwise interval & Simultaneous band \\',r'\midrule']
    for name in selected:
        ...
        if len(rows) and np.isfinite(rows.iloc[0].simultaneous_lower):
            row=rows.iloc[0];band=f'$[{row.simultaneous_lower:.4f}, {row.simultaneous_upper:.4f}]$';point=f'$[{row.lower:.4f}, {row.upper:.4f}]$'
        lines.append(f'{LABELS.get(name,name)} & {r.QS_x10000:.4f} & {100*r.violation_rate:.3f} & {uc} & {int(r.worse_than_raw)} & {point} & {band}'+r' \\')
    lines += [r'\bottomrule',r'\end{tabular}',r'\endgroup'];write('tab_ten_strong.tex','\n'.join(lines))
