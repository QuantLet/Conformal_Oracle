...
import numpy as np
import pandas as pd
from validate_full import PROJECT, ROOT, EXT, CHRONOS, MAP, sha, metric, check_metric, check_close, BACKTEST_ROUNDOFF
...
def main():
    ...
    assert complete['prefix_checks'] == 216
    ...
    assert len(metrics) == 1944 and metrics.index.is_unique
    ...
    for asset in assets:
        ...
        for model in models:
            ...
            if model in ['PatchTST-FM', 'Chronos-2']:
                native = np.load((ROOT/'patchtst_full' if model == 'PatchTST-FM' else CHRONOS)/f'{asset}.npz')
                ...
                if model == 'Lag-Llama':
                    fp = PROJECT/'artifacts/review_20260909/calendar/data/lagllama'/f'{asset}.parquet'
    ...
    result = dict(status='passed', producer_sha256=sha(__file__), complete_sha256=sha(OUT/'complete.json'),
        ...
        independent_metric_checks=metric_checks, inner_bootstrap_resamples=216*499*2,
