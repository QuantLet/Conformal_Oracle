...
OUT = PROJECT / 'artifacts/r8_team_review'
...
def verify_extension(path, prefix, hashes, check):
    ...
    names.update(p for p in hashes if p.startswith((
        'research/r8_team_review/', 'artifacts/r8_team_review/')))
    names.add(check['ewma_current_replay']['producer'])
    for row in check['ewma_current_replay']['rows']:
    ...
    with zipfile.ZipFile(path) as archive, tempfile.TemporaryDirectory(prefix='irfa-info-release-') as folder:
        ...
        run = subprocess.run([sys.executable, 'research/r8_team_review/validate.py'],
                             cwd=folder, capture_output=True, text=True)
        assert run.returncode == 0, (run.stdout, run.stderr)
        replay = json.loads((Path(folder) / 'artifacts/r8_team_review/final_validation.json').read_text())
        assert replay == check, 'Archive-only integration replay differs'
    return len(names)


def main():
    previous = json.loads((PROJECT / 'artifacts/r8_novelty_audit/full_release.json').read_text())
    ...
    prefix = 'R8_20260910_team_review'
    ...
    overlays = {}
    for root in ['research/r8_team_review', 'artifacts/r8_team_review',
                 'research/r8_concision', 'artifacts/r8_concision', 'docs/team_review_20260910']:
    ...
    extras.update(['docs/IRFA_CONCISION.md', 'docs/IRFA_NOVELTY_MATH_AUDIT.md', 'docs/IRFA_EXTERNAL_MATH_REVIEW_BRIEF.md',
                   'docs/IRFA_REVIEW_STATE.md', 'artifacts/r8_novelty_audit/full_release.json',
    ...
    overlays['README.md'] = PROJECT / 'research/r8_team_review/README.md'
    ...
    with zipfile.ZipFile(prior) as old:
        ...
        with zipfile.ZipFile(partial, 'w', allowZip64=True) as new:
            ...
            historical = 'historical/novelty_audit_release_manifest.json'
            content = old.read(manifest_name)
            new.writestr(prefix + '/' + historical, content)
            hashes[historical] = hashlib.sha256(content).hexdigest()
            record = dict(revision='R8', extension='100 focused AI reviews and current document validation',
    ...
    receipt = dict(status='complete', archive=str(target.relative_to(PROJECT)), archive_sha256=sha(target),
                   ...
                   focused_ai_reviews_completed=100, historical_study_archives_preserved=True,
