...
from prepare import PROJECT,OUT,ASSETS,YEARS,sha,prepare
...
def main():
    ...
    for asset in ASSETS:
        ...
        for name in ['CAViaR-AS','GAS-t']:
            ...
            assert done['binding']['producer_sha256']==sha(PROJECT/'research/r8_external2/dynamic.py')
    ...
    record=dict(status='passed',producer_sha256=sha(__file__),admission_sha256=sha(OUT/'admission.json'),
                ...
                base_series=4*len(ASSETS),full_fresh_replays=4*len(ASSETS),receipts=refs)
