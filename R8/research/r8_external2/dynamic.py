"""Annual past-only CAViaR-AS/GAS-t fits using the existing optimisers (Developed ex-US 25 universe)."""
import os
...
from prepare import PROJECT, OUT, ASSETS, YEARS, sha
...
def work(asset,name,replay=False,years=None):
    ...
    if years is None:
        ...
        (base/'complete.json').write_text(json.dumps(dict(binding=binding,forecast_sha256=sha(target),
            years={str(y):sha(base/f'{y}.json') for y in YEARS},rows=len(all_f)),indent=2)+'\n')
