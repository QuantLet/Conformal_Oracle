...
from prepare import END_LABEL
PAIRS=len(MODELS)*len(ASSETS)
...
def main():
    ...
    primary=json.loads((results/'primary_decisions.json').read_text());families=[('correction',['State-L1','POT-Shift','POT-Vol','DtACI-projected-expected'],'Shift-CP'),('indication',['Gate-selected-rolling','Loss-gate','Past-minimum'],'Selected-rolling'),('reference',['Shift-CP'],'Raw')]
    ...
    for name,g in table.groupby('method',sort=False):
        s=summary.loc[name];assert s.pairs==PAIRS
    ...
    for label,lo,hi in [('2015-2019',2015,2019),('2020-2021',2020,2021),(f'2022-{END_LABEL}',2022,2100)]:
    ...
    checks=0
    for block in [20,60]:
        seed=int.from_bytes(hashlib.sha256(f'20260913/panel-calendar/{block}'.encode()).digest()[:4],'little');rng=np.random.default_rng(seed)
        recreated=[]
        for start in range(0,999,25):
            ...
            counts=np.array(counts);means=(counts@values.reshape(len(index),-1)).reshape(len(counts),PAIRS,len(METHODS))/counts.sum(1)[:,None,None]
    ...
    record=dict(status='passed',producer_sha256=sha(__file__),result_complete_sha256=sha(results/'complete.json'),
                pairs=PAIRS,methods=len(METHODS),daily_losses_rebuilt=int(values.size),independent_calendar_draws=1998,
