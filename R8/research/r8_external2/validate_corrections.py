"""Independent losses, ranks, selections and mixtures, plus all 100 fresh replays."""
import hashlib
import json
import math
import sys
...
def main():
    ...
    perturbations=[]
    for model in MODELS:
        key=f'{model}__{ASSETS[0]}';y,q,sigma,index,nc,_=load(model,ASSETS[0]);changed=y.copy();changed[nc:]+=np.linspace(.1,1,len(y)-nc)
    ...
    record=dict(status='passed',producer_sha256=sha(__file__),pairs=len(MODELS)*len(ASSETS),full_fresh_replays=len(MODELS)*len(ASSETS),independent_scalar_checks=scalars,
