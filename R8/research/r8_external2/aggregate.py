"""External primary endpoint and the three protocol-fixed simultaneous families (Developed ex-US 25 universe)."""
import os
...
SEED_PREFIX='20260913'  # protocol-fixed calendar-bootstrap seed for this universe


def calendar_seed(block):return int.from_bytes(hashlib.sha256(f'{SEED_PREFIX}/panel-calendar/{block}'.encode()).digest()[:4],'little')
...
def arrays():
    ...
    for model in MODELS:
        for asset in ASSETS:
            ...
            for label,low,high in [('2015-2019',2015,2019),('2020-2021',2020,2021),(f'2022-{END_LABEL}',2022,2100)]:
    ...
    assert len(frames)==PAIRS and all(f.index.equals(frames[0].index) for f in frames)
...
def main():
    ...
    rows=[]
    for block in [20,60]:
        rng=np.random.default_rng(calendar_seed(block));draws=[]
        for _ in range(999):
    ...
    (RESULTS/'complete.json').write_text(json.dumps(dict(status='complete',producer_sha256=sha(__file__),pairs=PAIRS,methods=METHODS,seed_prefix=SEED_PREFIX,
