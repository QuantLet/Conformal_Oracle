...
OUT = PROJECT / 'artifacts/r8_shape_integration'
...
def verify_extension(path, prefix, hashes, check):
    ...
    names.update(p for p in hashes if p.startswith((
        'research/r8_shape_integration/', 'artifacts/r8_shape_integration/')))
    names.update('source/' + doc + '.log' for doc in ('main_R8', 'supplement_R8'))
    with zipfile.ZipFile(path) as archive:
        portable_bytes = archive.read(prefix + '/' + check['documents']['source_package']['source_zip'])
        with zipfile.ZipFile(io.BytesIO(portable_bytes)) as portable:
            names.update('source/' + name for name in portable.namelist() if name != 'BUILD.txt')
    assert names.issubset(hashes), sorted(names - set(hashes))
    with zipfile.ZipFile(path) as archive, tempfile.TemporaryDirectory(prefix='irfa-info-release-') as folder:
        for name in names:
        ...
        run = subprocess.run([sys.executable, 'research/r8_shape_integration/validate.py'],
                             cwd=folder, capture_output=True, text=True)
        assert run.returncode == 0, (run.stdout, run.stderr)
        replay = json.loads((Path(folder) / 'artifacts/r8_shape_integration/final_validation.json').read_text())
        assert replay == check, 'Archive-only integration replay differs'
    return len(names)


def main():
    previous = json.loads((PROJECT / 'artifacts/r8_contiguous_integration/full_release.json').read_text())
    ...
    prefix = 'R8_20260911_shape_integration'
    ...
    overlays = {}
    for root in ['research/r8_shape_integration', 'artifacts/r8_shape_integration',
                 'docs/shape_integration_20260911']:
    ...
    with zipfile.ZipFile(PROJECT / check['documents']['source_package']['source_zip']) as portable:
        extras.update('source/' + name for name in portable.namelist() if name != 'BUILD.txt')
    extras.update(['docs/IRFA_CONCISION.md', 'docs/IRFA_NOVELTY_MATH_AUDIT.md', 'docs/IRFA_EXTERNAL_MATH_REVIEW_BRIEF.md',
                   'docs/IRFA_REVIEW_STATE.md', 'artifacts/r8_contiguous_integration/full_release.json',
    ...
    extras.update(str(path.relative_to(PROJECT)) for path in
                  (PROJECT / 'artifacts/r8_shape_integration').glob('portable_*.log'))
    for name in extras:
        overlays[name] = PROJECT / name
    overlays['README.md'] = PROJECT / 'research/r8_shape_integration/README.md'
    # Exercise the complete new validator from a small temporary overlay archive
    # before streaming the preceding 16 GB release.
    preflight_hashes={name:sha(path) for name,path in overlays.items()}
    with tempfile.TemporaryDirectory(prefix='irfa-shape-preflight-') as folder:
        candidate=Path(folder)/'preflight.zip'
        with zipfile.ZipFile(candidate,'w',allowZip64=True) as archive:
            for name,path in overlays.items():archive.write(path,prefix+'/'+name)
        count=verify_extension(candidate,prefix,preflight_hashes,check)
        print('Preflight archive-only replay passed:',count,'files',flush=True)
    ...
    with zipfile.ZipFile(prior) as old:
        ...
        with zipfile.ZipFile(partial, 'w', allowZip64=True) as new:
            ...
            historical = 'historical/contiguous_integration_release_manifest.json'
            content = old.read(manifest_name)
            new.writestr(prefix + '/' + historical, content)
            hashes[historical] = hashlib.sha256(content).hexdigest()
            record = dict(revision='R8', extension='authorised known-scale correction-shape theory, simulation and financial context',
    ...
    receipt = dict(status='complete', archive=str(target.relative_to(PROJECT)), archive_sha256=sha(target),
                   ...
                   preceding_focused_reviews_preserved=100, shape_cost_integrated=True, historical_study_archives_preserved=True,
