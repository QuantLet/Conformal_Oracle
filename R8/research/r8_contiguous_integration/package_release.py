...
OUT = PROJECT / 'artifacts/r8_contiguous_integration'
...
def verify_extension(path, prefix, hashes, check):
    ...
    names.update(p for p in hashes if p.startswith((
        'research/r8_contiguous_integration/', 'artifacts/r8_contiguous_integration/')))
    ...
    with zipfile.ZipFile(path) as archive, tempfile.TemporaryDirectory(prefix='irfa-info-release-') as folder:
        ...
        run = subprocess.run([sys.executable, 'research/r8_contiguous_integration/validate.py'],
                             cwd=folder, capture_output=True, text=True)
        assert run.returncode == 0, (run.stdout, run.stderr)
        replay = json.loads((Path(folder) / 'artifacts/r8_contiguous_integration/final_validation.json').read_text())
        assert replay == check, 'Archive-only integration replay differs'
    return len(names)


def main():
    previous = json.loads((PROJECT / 'artifacts/r8_team_review/full_release.json').read_text())
    ...
    prefix = 'R8_20260911_contiguous_integration'
    ...
    overlays = {}
    for root in ['research/r8_contiguous_integration', 'artifacts/r8_contiguous_integration',
                 'docs/contiguous_integration_20260911']:
    ...
    extras.update(['docs/IRFA_CONCISION.md', 'docs/IRFA_NOVELTY_MATH_AUDIT.md', 'docs/IRFA_EXTERNAL_MATH_REVIEW_BRIEF.md',
                   'docs/IRFA_REVIEW_STATE.md', 'artifacts/r8_team_review/full_release.json',
    ...
    extras.update(str(path.relative_to(PROJECT)) for path in
                  (PROJECT / 'artifacts/r8_contiguous_integration').glob('portable_*.log'))
    for name in extras:
        overlays[name] = PROJECT / name
    overlays['README.md'] = PROJECT / 'research/r8_contiguous_integration/README.md'
    ...
    with zipfile.ZipFile(prior) as old:
        ...
        with zipfile.ZipFile(partial, 'w', allowZip64=True) as new:
            ...
            historical = 'historical/team_review_release_manifest.json'
            content = old.read(manifest_name)
            new.writestr(prefix + '/' + historical, content)
            hashes[historical] = hashlib.sha256(content).hexdigest()
            record = dict(revision='R8', extension='authorised contiguous marginal coverage and newsvendor comparison',
    ...
    receipt = dict(status='complete', archive=str(target.relative_to(PROJECT)), archive_sha256=sha(target),
                   ...
                   preceding_focused_reviews_preserved=100, contiguous_coverage_integrated=True, historical_study_archives_preserved=True,
