...
import tempfile
import zipfile
from build import ROOT as PROJECT, OUT
...
def verify_extension(path, prefix, hashes, check):
    ...
    names.update(p for p in hashes if p.startswith((
        'research/r8_information_integration/', 'artifacts/r8_information_integration/',
        'research/r8_information_limit/', 'artifacts/r8_information_limit/')))
    names.add('release/R8_20260910_information_limit.zip')
    ...
    with zipfile.ZipFile(path) as archive, tempfile.TemporaryDirectory(prefix='irfa-info-release-') as folder:
        ...
        run = subprocess.run([sys.executable, 'research/r8_information_integration/validate.py', '--replay-study'],
                             cwd=folder, capture_output=True, text=True)
        assert run.returncode == 0, (run.stdout, run.stderr)
        replay = json.loads((Path(folder) / 'artifacts/r8_information_integration/final_validation.json').read_text())
        assert replay == check, 'Archive-only integration replay differs'
    return len(names)


def main():
    previous = json.loads((PROJECT / 'artifacts/r8_partial_integration/full_release.json').read_text())
    ...
    prefix = 'R8_20260910_information_integration'
    ...
    overlays = {}
    for root in ['research/r8_information_limit', 'artifacts/r8_information_limit',
                 'research/r8_information_integration', 'artifacts/r8_information_integration']:
    ...
    extras.update(['docs/IRFA_INFORMATION_LIMIT_RESULTS.md', 'docs/IRFA_INFORMATION_INTEGRATION.md',
                   'docs/IRFA_REVIEW_STATE.md', 'artifacts/r8_partial_integration/full_release.json',
                   'release/R8_20260910_information_limit.zip',
    ...
    overlays['README.md'] = PROJECT / 'research/r8_information_integration/README.md'
    ...
    with zipfile.ZipFile(prior) as old:
        ...
        with zipfile.ZipFile(partial, 'w', allowZip64=True) as new:
            ...
            historical = 'historical/partial_integration_release_manifest.json'
            content = old.read(manifest_name)
            new.writestr(prefix + '/' + historical, content)
            hashes[historical] = hashlib.sha256(content).hexdigest()
            record = dict(revision='R8', extension='information lower bound integrated',
