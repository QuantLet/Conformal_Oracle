...
import tempfile
import zipfile
from check import ROOT as PROJECT, OUT
...
def verify_extension(path, prefix, hashes, check):
    ...
    names.update(p for p in hashes if p.startswith((
        'research/r8_novelty_audit/', 'artifacts/r8_novelty_audit/')))
    names.add('artifacts/r8_count_law/exact_witness.json')
    ...
    with zipfile.ZipFile(path) as archive, tempfile.TemporaryDirectory(prefix='irfa-info-release-') as folder:
        ...
        run = subprocess.run([sys.executable, 'research/r8_novelty_audit/validate.py'],
                             cwd=folder, capture_output=True, text=True)
        assert run.returncode == 0, (run.stdout, run.stderr)
        replay = json.loads((Path(folder) / 'artifacts/r8_novelty_audit/final_validation.json').read_text())
        assert replay == check, 'Archive-only integration replay differs'
    return len(names)


def main():
    previous = json.loads((PROJECT / 'artifacts/r8_information_integration/full_release.json').read_text())
    ...
    prefix = 'R8_20260910_novelty_audit'
    ...
    overlays = {}
    for root in ['research/r8_novelty_audit', 'artifacts/r8_novelty_audit']:
    ...
    extras.update(['docs/IRFA_NOVELTY_MATH_AUDIT.md', 'docs/IRFA_EXTERNAL_MATH_REVIEW_BRIEF.md',
                   'docs/IRFA_REVIEW_STATE.md', 'artifacts/r8_information_integration/full_release.json',
    ...
    overlays['README.md'] = PROJECT / 'research/r8_novelty_audit/README.md'
    ...
    with zipfile.ZipFile(prior) as old:
        ...
        with zipfile.ZipFile(partial, 'w', allowZip64=True) as new:
            ...
            historical = 'historical/information_integration_release_manifest.json'
            content = old.read(manifest_name)
            new.writestr(prefix + '/' + historical, content)
            hashes[historical] = hashlib.sha256(content).hexdigest()
            record = dict(revision='R8', extension='novelty positioning and alternative mathematical audit',
    ...
    receipt = dict(status='complete', archive=str(target.relative_to(PROJECT)), archive_sha256=sha(target),
                   ...
                   alternative_mathematical_audit_repeated=True, historical_study_archives_preserved=True,
