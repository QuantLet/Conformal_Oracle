...
from run import OUT as DATA,RESULTS as BASE_RESULTS,MODELS,ASSETS,METHODS,sha
...
def main():
    ...
    pairs=pd.read_csv(BASE_RESULTS/'pairs.csv');assert len(pairs)==240*20
    assert not pairs.duplicated(['model','asset','method']).any()
    summary=pd.read_csv(BASE_RESULTS/'summary.csv').set_index('method')
    scalar_checks=0
    ...
    for asset in ASSETS:
        ...
        for model in MODELS:
            key=f'{model}__{asset}';folder=DATA/'pairs'/key
    ...
    result=dict(status='passed',producer_sha256=sha(__file__),complete_sha256=sha(RESULTS/'complete.json'),
        pairs=240,methods=20,simultaneous_family_size=8,independent_scalar_checks=scalar_checks,independent_calendar_draws=1998,
