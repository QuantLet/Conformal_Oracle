...
import zipfile
import sys
sys.path.insert(0,str(Path(__file__).resolve().parents[1]/'r8_ten_integration'))
from build import PROJECT,SOURCE,check
...
def main():
    ...
    assert json.loads((OUT/'gbm/validation.json').read_text())['status']=='passed'
    assert json.loads((OUT/'aggregation_validation.json').read_text())['status']=='passed'
    name='R8_20260910_referee_revision';target=PROJECT/'release'/f'{name}.zip'
    ...
    trees=['research/r8_referee_revision','artifacts/r8_referee_revision','source/sections_r8','source/analysis/provenance_r8','source/scripts/extension_20260831',
    ...
    for rel in ['source/calibrating_the_oracle.bib','source/scripts/build_guards.py','docs/IRFA_REVIEW_STATE.md','docs/IRFA_REFEREE_RESPONSE.md','docs/IRFA_TEN_MODEL_COMPLETION.md',
    ...
    for manifest in [PROJECT/'artifacts/r8_commodity_etp/panel/base/results/paper_outputs_manifest.json',
                     ...
                     PROJECT/'artifacts/r8_regime/paper_displays.json',PROJECT/'artifacts/r8_ten_integration/displays.json']:
