...
P = .99
K_FOLDS = 5
# Amendment of 13 September 2026, dated after the original run (archived in
# artifacts/r8_optimism_estimator_original_4over5): E[L_cv - I_n] = A_m + A_n
# with m = n(K-1)/K, so the first-order factor mapping it to 2A_n is
# 2(K-1)/(2K-1) = 8/9, not (K-1)/K = 4/5. iid leading-order justification only.
CV_FACTOR = 2 * (K_FOLDS - 1) / (2 * K_FOLDS - 1)
R_BOOT = 200
SEED = 20260913
HISTORIES = 500
