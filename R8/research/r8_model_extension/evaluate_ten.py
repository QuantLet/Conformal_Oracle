...
OUT = ROOT/'ten_common_evaluation'
...
MODELS = ['PatchTST-FM', 'Chronos-2', 'TS-ICL', 'Moirai-1.1', 'Lag-Llama', 'GJR-GARCH',
...
FAMILY += [('TS-ICL Static - Raw','TS-ICL/Static','TS-ICL/Raw'),
           ('TS-ICL Rolling250 - Raw','TS-ICL/Rolling250','TS-ICL/Raw')]
FAMILY += [(f'TS-ICL - {model} {method}', f'TS-ICL/{method}', f'{model}/{method}')
           for model in ['PatchTST-FM','Chronos-2'] for method in ['Raw','Static']]
...
def forecast(model, asset, dates):
    if model in MODELS[:3]:
        folder = ROOT / {'PatchTST-FM':'patchtst_full', 'Chronos-2':'chronos2_full', 'TS-ICL':'tsicl_assembled'}[model]
...
def main():
    ...
    sources = [Path(__file__), Path(__file__).with_name('PROTOCOL.md'), ROOT/'full_preflight/support.csv',
               ...
               ROOT/'preparation_ten.json', Path(__file__).with_name('scope.py')]
    binding = {str(p.relative_to(PROJECT)): sha(p) for p in sources}
    metrics, boundaries, calibration, daily = [], [], [], {}
    preparation = json.loads((ROOT/'preparation_ten.json').read_text())
    ...
    assert len(table) == 720 and table.n_test.sum() == int(support.n_test.sum())*30
    ...
    dump(OUT/'complete.json', dict(status='complete', binding=binding, outputs=outputs, models=MODELS,
        methods=METHODS, assets=len(assets), pairs=240, asset_test_observations=int(support.n_test.sum()),
