...
import numpy as np
import pandas as pd
from evaluate import PROJECT, ROOT, EXT, MODELS, forecast, sha, dump, scores
...
def main():
    ...
    sources = [Path(__file__), Path(__file__).with_name('PROTOCOL.md'),
        Path(__file__).with_name('evaluate.py'), Path(m.__file__),
    ...
    dump(OUT/'complete.json', dict(status='complete', binding=binding, protected=protected, outputs=outputs,
        models=MODELS, methods=METHODS, family=FAMILY, pairs=216, rows=1944, asset_test_dates=int(support.n_test.sum()),
