...
## Extension of 16 September 2026

`source/editorial/render_tables.py` joins the replay as step 22, the last
step. It binds by SHA-256 two archived aggregate files (copies of the
ten-comparator `summary.csv` and the referee-revision `intervals.csv`) and
the two base tables regenerated by earlier steps, then writes two
presentation derivatives: `tab_ten_strong_complete.tex`, which adds the
existing projected-DtACI row to the stronger-comparison table, and
`tab_controlled_dimensions.tex`, which writes the coefficient-count symbol
