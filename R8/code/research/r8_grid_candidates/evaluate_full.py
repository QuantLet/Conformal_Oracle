...
def main():
    ...
    sources = [Path(__file__), Path(__file__).with_name('FULL_PROTOCOL.md'), ROOT/'full_preflight/support.csv',
               PROJECT/'source/scripts/extension_20260831/panel_statistics.py',
               ROOT/'patchtst_full/complete.json', CHRONOS/'complete.json', CHRONOS/'replay.json']
    binding = {str(p.relative_to(PROJECT)): sha(p) for p in sources}
    metrics, boundaries, calibration, daily = [], [], [], {}
    patch_receipt = json.loads((ROOT/'patchtst_full/complete.json').read_text())
    assert patch_receipt['status'] == 'complete'
    chronos_receipt = json.loads((CHRONOS/'complete.json').read_text())
    for receipt, folder in [(patch_receipt, ROOT/'patchtst_full'), (chronos_receipt, CHRONOS)]:
        for item in receipt['assets']:
            assert sha(folder/f"{item['asset']}.npz") == item['output_sha256']
    ...
    assert len(table) == 648 and table.n_test.sum() == 37483*27
    ...
    dump(OUT/'complete.json', dict(status='complete', binding=binding, outputs=outputs, models=MODELS,
        methods=METHODS, assets=len(assets), pairs=216, asset_test_observations=37483,
