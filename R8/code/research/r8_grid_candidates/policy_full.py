...
import numpy as np
import pandas as pd
from evaluate_full import PROJECT, ROOT, EXT, MODELS, forecast, sha, dump, scores
...
OUT = ROOT/'policy_evaluation'
COMMON = ROOT/'common_evaluation'
...
def main():
    ...
    sources = [Path(__file__), Path(__file__).with_name('POLICY_PROTOCOL.md'),
        Path(__file__).with_name('evaluate_full.py'), Path(m.__file__),
    ...
    for asset in support.index:
        ...
        print(asset, '9 policy decisions and prefix checks complete', flush=True)
    metrics = pd.DataFrame(rows); assert len(metrics) == 1944
    ...
    dump(OUT/'complete.json', dict(status='complete', binding=binding, protected=protected, outputs=outputs,
        models=MODELS, methods=METHODS, family=FAMILY, pairs=216, rows=1944, asset_test_dates=37483,
        prefix_checks=216, packages={p: md.version(p) for p in ['numpy', 'pandas', 'scipy', 'pyarrow']}))
