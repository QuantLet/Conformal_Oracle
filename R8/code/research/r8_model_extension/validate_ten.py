...
def main():
    ...
    assert len(metrics) == 720 and metrics.index.is_unique
    ...
    result.update(status='passed', full_grid_rows=grid_rows, full_grid_native_values=grid_rows*99,
        ...
        fresh_full_replay_exact=True, independent_metric_checks=checks, evaluated_pairs=240,
        evaluated_pair_methods=720, asset_test_dates=preparation['asset_test_dates'], independently_verified_bootstrap_draws=1998,
        independent_bootstrap_mean_values=1998*30, protected_canonical_files_unchanged=len(protected),
        backtest_roundoff_max=BACKTEST_ROUNDOFF, backtest_absolute_tolerance=1e-10,
        backtest_rejection_decisions_exact=True,
        complete_sha256=sha(OUT/'complete.json'), preparation_sha256=sha(ROOT/'preparation_ten.json'))
