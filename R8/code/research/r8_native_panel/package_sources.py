...
OUT = PROJECT / 'artifacts/r8_native_panel'
...
def main():
    ...
    previous = release / 'R8_20260910_ijf_argument_sources.zip'
    target = release / 'R8_20260910_native_sources.zip'
    ...
    with tempfile.TemporaryDirectory(prefix='irfa-native-sources-') as folder:
        ...
        for doc in ['main_R8', 'supplement_R8']:
            ...
            built = work / (doc + '.pdf')
            texts = [subprocess.check_output(['pdftotext', str(p), '-'], text=True) for p in [current, built]]
    ...
    (release / 'R8_native_source_receipt.json').write_text(json.dumps(receipt, indent=2) + '\n')
