    ...
    for block in BLOCKS + SENSITIVITY_BLOCKS:
        ...
        if block in BLOCKS:
            ...
            sign_rows.append({'block_calendar_days': block, 'draws': DRAWS, 'pairs_improved': n_improved, 'pairs': 240,
                              ...
                              'binomial_p_two_sided_reference_only': float(2 * min(_binom_cdf(n_improved, 240), _binom_upper_tail(n_improved, 240)))})
        else:
...
def write(result):
    ...
    (OUT / 'run.json').write_text(json.dumps({
        ...
        'seed_convention': "int.from_bytes(sha256('20260909/panel-calendar/{block}/0')[:4], 'little')",
