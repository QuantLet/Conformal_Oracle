...
\begin{document}
	...
	\begin{frontmatter}
		...
		%─── PART 2: Abstract ───────────────────────────────────────────────────────
		\begin{abstract}
			Probabilistic forecasts used in financial risk management must be
			well-calibrated, yet many models produce systematically biased tail
			risk estimates. This paper proposes a distribution-free
			post-processing framework that recalibrates Value-at-Risk forecasts
			using one-sided conformal prediction, requiring no distributional
			assumptions or model retraining. An approximate coverage guarantee
			is established under $\beta$-mixing dependence. The scalar conformal
			threshold $\hat{q}_V$ serves both as the corrective shift restoring
			coverage and as a continuous, signed diagnostic of tail
			miscalibration severity.
	...
	Within this post-processing paradigm, the output of a fixed
	forecaster is adjusted without retraining
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=\textwidth]{calibration_efficiency_frontier_v2}
		\caption{Calibration--efficiency frontier for four representative forecasters. \textbf{(a)}~Zoomed view near the 99\% target. \textbf{(b)}~Full scale including Chronos-Small (raw $\hat{\pi} = 38.8\%$). Each arrow traces one forecaster from its raw (hollow) to its conformally corrected (filled) position.}
	...
	independent.\footnote{During crisis periods, cross-asset
	...
	\begin{table}[H]
		...
		\begin{minipage}{\linewidth}\footnotesize
			...
			crisis-period cross-asset correlation), 25 of 36 remain
	...
	S\&P~500 under three configurations: (i)~Lag-Llama in the
	Yellow zone ($k = 3.65$) applied to the corrected VaR,
	(ii)~conformally corrected Lag-Llama in the Green zone
	($k = 3.00$), and (iii)~GJR-GARCH with conformal correction
	($k = 3.00$). Over the 1{,}824-day test window (2018--2026),
	the cumulative capital saving from reclassifying Lag-Llama from
	Yellow to Green amounts to 17.8\% of the Yellow-zone cumulative
	charge---exactly the multiplier ratio
	$(3.65 - 3.00)/3.65$. This saving materialises purely from the
	zone reclassification and requires no change to the underlying
	model (Figure~\ref{fig:capital_charge}). This saving is
	mechanical: it equals exactly the multiplier ratio
	$(3.65 - 3.00)/3.00 = 21.7\%$ of the Green-zone charge, or
	equivalently $(3.65 - 3.00)/3.65 = 17.8\%$ of the Yellow-zone
	charge, confirming that the corrected and uncorrected Lag-Llama
	produce nearly identical VaR widths on S\&P~500. The economic
	value of the conformal layer therefore lies in regulatory
	compliance---zone reclassification---rather than interval
	shrinkage. For models where the correction also tightens the
	interval (GJR-GARCH, Table~\ref{tab:master}), the capital
	saving exceeds the pure multiplier effect.
	...
	\begin{figure}[htbp]
		...
		VaR. The grey band marks the COVID-19 period (Feb--Apr~2020).}
	...
	$\approx w/2$ days of a crisis remain under-protected; a
	...
	For practitioners, GJR-GARCH with conformal correction
	dominates for standalone 1\% VaR on individual assets
	($|\qV| < 0.01$, tightest width, indistinguishable from best
	TSFM in DM tests). TSFMs add value for zero-shot deployment
	across large asset universes, ensemble forecasting, and
	distributional forecasting beyond VaR (the $|\qV|/|\VaR|$
	ratio in Section~\ref{sec:results_raw} provides a practical decision rule).

	\begin{description}
		\item[Single-asset regulatory VaR:] GJR-GARCH + static conformal correction. Lowest width, indistinguishable QS from best TSFM.
		\item[Zero-shot deployment across large asset universes:] TSFM (TimesFM or Lag-Llama) + rolling conformal correction. No per-asset estimation required.
		\item[Ensemble / distributional forecasting:] Combine TSFM forecasts with conformal layer as quality filter; use $|\hat{q}_V|/|\mathrm{VaR}|$ to screen out replacement-regime models.
		\item[Screening tool:] Use $\hat{q}_V$ ranking (Table~\ref{tab:master}) to select base forecasters before deployment; models with $|\hat{q}_V| > |\mathrm{VaR}|$ contribute no predictive content beyond a historical quantile.
	\end{description}
	...
	applications of this framework. The placeholder results in
	Table~\ref{tab:es_correction} provide an empirical starting
	point. The framework applies to univariate VaR by
