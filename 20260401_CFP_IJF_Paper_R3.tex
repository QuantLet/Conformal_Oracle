...
\usepackage{booktabs,colortbl,xcolor,array,multirow,threeparttable}
...
\begin{document}
	...
	\begin{frontmatter}
		...
		\author[1]{Andrei Theodor Ginavar}
		...
		%─── PART 2: Abstract ───────────────────────────────────────────────────────
		\begin{abstract}
			Tail risk forecasts from heterogeneous models---classical and
			foundation-model-based---exhibit systematic miscalibration that a
			single scalar functional can diagnose and correct. This paper
			proposes a distribution-free post-processing framework that
			recalibrates Value-at-Risk forecasts using one-sided conformal
			prediction, requiring no distributional assumptions or model
			retraining. An approximate coverage guarantee is established under
			$\beta$-mixing dependence. The scalar conformal threshold
			$\hat{q}_V$ serves both as the corrective shift restoring coverage
			and as a continuous, signed diagnostic of tail miscalibration
			severity.
			
			The framework is evaluated on five time series foundation models
			and four classical benchmarks across 24 financial assets over
			2000--2026. Conformal recalibration restores nominal coverage
			across the large majority of model--asset combinations (86.6\%
			Basel Green Zone, zero Red), revealing a clear
			calibration--efficiency trade-off. A rolling variant adapts to
			local market conditions, further improving conditional coverage.
	...
	Reliable probabilistic forecasting requires not only sharp
	predictions but also correct calibration: predicted quantiles
	must be statistically consistent with realised outcomes. In
	financial risk management, this requirement is particularly
	stringent. Value-at-Risk (VaR) forecasts are subject to formal
	regulatory backtesting \citep{basel1996}, and systematic
	deviations from nominal coverage translate directly into
	capital penalties. Yet, both classical econometric models and
	modern machine learning approaches routinely produce
	miscalibrated tail forecasts, especially under temporal
	dependence and regime shifts
	\citep{christoffersen1998, nolde2017elicitability,
		goel2024tsfmvar, pele2025llmvar}.
	
	This paper addresses a fundamental forecasting problem: how to
	transform arbitrary probabilistic forecasts into valid risk
	forecasts without modifying the underlying model. We propose a
	one-sided conformal correction that operates as a model-agnostic
	post-processing layer---requiring neither retraining nor
	distributional assumptions---and targets calibration rather than
	predictive accuracy. In this sense, it belongs to the forecast
	post-processing tradition of \citet{gneiting2013combining},
	\citet{brehmer2021properification}, and
	\citet{taillardat2016calibrated}, while focusing specifically
	on tail risk measures under dependence.
	
	The contribution is threefold. First, we introduce the one-sided
	conformal recalibration procedure and establish an approximate
	coverage result under $\beta$-mixing conditions, providing
	asymptotic support without relying on i.i.d.\ assumptions.
	Second, we conduct a large-scale empirical evaluation across
	five time series foundation models (TSFMs), four parametric
	benchmarks, and 24 financial assets over 2000--2026. Third, we
	show that the scalar conformal threshold $\qV$ (the empirical
	quantile of calibration-set forecast errors; see
	Definition~\ref{def:var_correction}) doubles as a continuous,
	signed miscalibration diagnostic---and document the main limitations, including
	sensitivity to calibration window size, persistence, and
	structural breaks.
	
	Data, code, and regular updates for this paper are available via the Quantlet \quantlet{https://github.com/QuantLet/Conformal_Oracle/} and Quantinar \quantinar{https://github.com/QuantLet/Conformal_Oracle/} platforms.
	...
	correction approaches. This study provides the first systematic
	calibration audit of time series foundation models for financial
	tail risk, covering five architectures across 24 assets over a
	...
	exchangeability is violated under volatility clustering.
	...
	We consider a two-layer forecasting framework.
	In the first layer, a base model~$M$ produces
	probabilistic forecasts of returns
	(Section~\ref{sec:layer1}).
	In the second layer, a calibration procedure adjusts
	these forecasts to ensure correct coverage of tail
	risk measures (Section~\ref{sec:layer2}).
	The objective is not to modify the
	underlying model, but to restore the reliability of
	its predicted quantiles through a minimal
	post-processing step.
	
	We use the following notation throughout. Let $r_t$
	denote the daily log return and
	$\hat{q}_t^{\,lo} = (\hat{F}_t^M)^{-1}(\alpha)$ the
	predicted lower $\alpha$-quantile from a base model~$M$.
	The calibration and test sets are obtained by a temporal
	split of the forecast outputs; no forecasts are
	re-estimated.
	
	\subsection{Layer~1: Base Probabilistic Forecasting}
	\label{sec:layer1}
	
	\subsubsection{TSFM-VaR Setup}
	\label{sec:setup}
	
	A foundation model $M$ generates a probabilistic forecast for
	the next-period return.
	
	For sample-based models (Chronos,
	...
	\end{align}
	Value-at-Risk and Expected Shortfall are computed as
	\citep{pele2025llmvar}:
	\begin{align}
	...
	the approximation is consistent as $N \to \infty$.
	For the baseline configuration ($N = 1{,}000$,
	$\alpha = 0.01$), the sum averages only
	$\lfloor N\alpha \rfloor = 10$ order statistics, making
	the ES estimate substantially noisier than the VaR
	estimate at the same level. This motivates our focus
	on VaR for the formal conformal analysis; a heuristic
	ES correction is deferred to Appendix \ref{app:es}.
	...
	\end{equation}
	where $I_t = \1(r_t \leq -\widehat{\VaR}_t^\beta)$. These
	...
	Forecast outputs are partitioned temporally into a calibration set and a test set; no base model is re-estimated. The
	calibration fraction is fixed at $f_c = 0.70$, with robustness examined in Section~\ref{sec:robust}.
	...
	\begin{definition}[Conformal VaR Correction]
		...
		nominal coverage. Larger values indicate systematic
	...
	The following result establishes approximate coverage under
	weak dependence, replacing the exchangeability assumption of
	...
	The approximation error $\Delta_n$ vanishes as the
	calibration sample increases, ensuring asymptotic coverage.
	...
	inherit the same formal guarantee but consistently delivers
	improved conditional coverage.

	\begin{proposition}[Local coverage of the rolling variant]
	...
	\begin{proof}
		Apply Theorem~\ref{thm:dependent_conformal_var} to the
	...
	backward-looking: at the onset of a volatility regime
	shift, the trailing window still contains pre-shift
	scores, and the correction lags the true change.
	For sudden, unprecedented
	events (e.g., March 2020), the lag is unavoidable.

	The rolling variant is related to Adaptive Conformal
	Inference \citep{gibbs2021adaptive}, but ACI's regret
	guarantee relies on symmetric miscoverage loss, which the
	one-sided VaR score breaks. More concretely, exponential
	weighting in the \citet{barber2023conformal} framework with
	decay parameter $\lambda$ applied to the one-sided score
	$s_t^V$ would yield a weighted quantile
	$\hat{q}_{V,t}^{w}$ that formally satisfies their weighted
	exchangeability guarantee (Theorem~2 of
	\citealt{barber2023conformal}). Combining this with the
	one-sided loss structure and deriving a one-sided regret
	bound analogous to ACI is a concrete open problem.
	...
	out-of-the-box calibration properties.
	
	All TSFM checkpoints predate the evaluation period;
	no model was updated or fine-tuned during evaluation.
	Chronos-Small and Chronos-Mini
	(v1.2, March~2024) were trained on a large public
	corpus; the authors report no financial price data
	in the training set \citep{ansari2024chronos}.
	TimesFM~2.5 (February~2025) was trained on Google
	Trends, synthetic series, and additional public
	datasets, with a training-data cutoff preceding~2024
	\citep{das2024timesfm}. Moirai~2.0 (October~2024)
	was trained on the LOTSA corpus, which includes
	selected financial series but with a training cutoff
	preceding our test period
	\citep{woo2024moirai,liu2024moirai2}.
	Lag-Llama (February~2024) was pre-trained on
	27~public datasets spanning energy, transportation,
	economics, nature, air quality, and cloud operations
	\citep{rasul2024lagllama}; the corpus includes an
	exchange-rate dataset but no equity, commodity,
	cryptocurrency, or bond series.
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}llrlll@{}}
			\toprule
			Model & Architecture & NN weights
			& Distribution & Forecast output & Context \\
			\midrule
			Chronos-Small  & Tokenisation   & 46M
			& Non-parametric & 1000 samples  & 512 obs.\ \\
			Chronos-Mini   & Tokenisation   & 20M
			& Non-parametric & 1000 samples  & 512 obs.\ \\
			TimesFM 2.5    & Decoder-only   & 200M
			& Non-parametric & Quantile grid & 512 obs.\ \\
			Moirai 2.0     & Masked encoder & 11.4M
			& Mixture        & 1000 samples  & 512 obs.\ \\
			Lag-Llama      & LLaMA-based    & 7.5M
			& Student-$t$    & 1000 samples  & 512 obs.\ \\
			\bottomrule
		\end{tabular}
		...
		\begin{tabular}{@{}lllll@{}}
			...
			& None ($\lambda = 0.94$ fixed)
			& Recursive\textsuperscript{$\dagger$} \\
		...
		\smallskip
		{\footnotesize
			\textsuperscript{$\dagger$}Exponential decay
			($\lambda = 0.94$); effective window
			$\approx 75$~obs.\ ($\lambda^{75} < 0.01$).}
	\end{table}
	
	\subsection{Experimental Design}
	
	Forecasts are generated using a rolling window. For
	TSFMs, the context length is set to 512~observations,
	the default input length for all five pretrained
	checkpoints (Table~\ref{tab:models}). For parametric
	benchmarks, the estimation window is 250~observations
	(one trading year), following the Basel regulatory
	convention
	\citep{basel1996}.
	The two windows serve different roles: the TSFM
	context is a fixed-length input to a pretrained model
	(no parameters are estimated), whereas the parametric
	window defines the sample over which model parameters
	are re-estimated at each step. The sample is split into a calibration set (70\%) and
	...
	dominates the forecast (indicated by $\dagger$ in
	...
	ranking of tail reliability. The ranking spans an order of
	...
	Conformal recalibration transforms all forecasters into
	effectively Basel-compliant risk models. Across 216 model--asset
	pairs, 187 achieve Green status (86.6\%), 29 fall into Yellow,
	and none enter Red (Table~\ref{tab:master}). This headline rate
	...
	To distinguish genuine recalibration from effective
	replacement, we disaggregate by the $|\qV|/|\VaR_{\mathrm{raw}}|$
	...
	In these cases, the conformal layer effectively defaults to a robust empirical quantile, providing a distribution-free safety mechanism rather than relying on misspecified model structure---a feature, not a limitation, of the model-agnostic design.

	Coverage is restored at a cost. VaR estimates widen, reflecting
	...
	\begin{figure}[htbp]
		...
		\caption{Calibration--efficiency frontier for four representative forecasters. Each arrow traces one forecaster from its raw (hollow) to its conformally corrected (filled) position. Conformal correction moves forecasts toward the 99\% coverage target at the cost of VaR width expansion. Chronos-Small enters from raw $\hat\pi = 38.8\%$ (inset). GJR-GARCH moves left---the only model where correction \emph{shrinks} the interval.}
	...
	independent.\footnote{During stress periods, cross-asset
		correlations spike and the effective cluster count falls below
		24, so $p_{\text{cluster}}$ may be anti-conservative under
		systemic stress. The high values in Table~\ref{tab:panel}
		(most $> 0.70$) provide a comfortable margin.} All TSFMs
	...
	\begin{table}[H]
		\centering
		\caption{Panel-pooled backtest by asset class ($\alpha = 0.01$)}
	...
	The conformal framework corrects heterogeneous miscalibration
	across TSFM architectures without requiring knowledge of the
	underlying failure mechanism. The strong correlation between
	$\qV$ and asset-level volatility and tail frequency
	(Table~\ref{tab:cross_sectional}) is consistent with
	...
	dynamics of the predictive distribution. The rolling variant
	substantially improves conditional coverage
	(Table~\ref{tab:rolling_vs_static}) but remains a location
	...
	multiplier ($k \in [3,4]$). The conformal correction trades a
	modest width increase (0.93--1.05$\times$ GJR-GARCH) for zone
	improvement from Red/Yellow to Green. For the median TSFM
	(Lag-Llama, W/GJR = 1.05), the corrected capital charge is
	$3 \times 1.05 = 3.15\times$ VaR versus $3.65\times$ VaR
	under the uncorrected Yellow classification---a net saving
	of 14\%.

	To illustrate the cumulative economic impact, consider the daily
	capital charge $k \times |\widehat{\mathrm{VaR}}_t|$ for
	S\&P~500 under three configurations: (i)~Lag-Llama at the
	Yellow-zone multiplier ($k = 3.65$), (ii)~conformally corrected
	Lag-Llama at the Green-zone multiplier ($k = 3.00$), and
	(iii)~GJR-GARCH with conformal correction ($k = 3.00$). Over
	the 1{,}824-day test window (2018--2026), reclassifying
	Lag-Llama from Yellow to Green saves 17.8\% of the cumulative
	charge (Figure~\ref{fig:capital_charge}). The saving is
	mechanical---it equals exactly the multiplier ratio
	$(3.65 - 3.00)/3.65$---confirming that the corrected and
	uncorrected Lag-Llama produce nearly identical VaR widths on
	this asset. The economic value of the conformal layer therefore
	lies in regulatory compliance rather than interval shrinkage.
	For models where the correction also tightens the interval
	(GJR-GARCH, Table~\ref{tab:master}), the capital saving exceeds
	the pure multiplier effect.
	...
	Figure~\ref{fig:covid_response} illustrates failure
	mode~(iv) empirically. During the COVID-19 shock
	(February--March 2020), S\&P~500 realised volatility
	peaked at $0.98$ (annualised), roughly $8\times$ its
	pre-shock level. The rolling $\hat{q}_{V,t}$ responds
	with a lag that varies across forecasters: the TSFMs
	(TimesFM, Moirai, Lag-Llama) peak approximately
	77~days after the volatility peak ($\approx w/3$),
	while the parametric benchmarks---whose base forecasts
	already adapt to conditional volatility---respond more
	slowly (GJR-GARCH: 161~days). All models show
	$\hat{q}_{V,t}$ increasing by $2.6$--$3.0\times$
	their pre-shock means, confirming
	that the rolling correction absorbs the regime change but
	with a structural delay inherent to any backward-looking
	window estimator. The lag implies that the first
	$\approx w/2$ days of a stress episode remain under-protected; a
	stressed calibration floor or a volatility-triggered
	override (e.g., $\qV \geq \qV^{\mathrm{stressed}}$ when
	realised volatility exceeds a threshold) would mitigate
	this in practice.
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=\textwidth]{fig_covid_response.pdf}
		\caption{Rolling 250-day conformal threshold $\hat{q}_{V,t}$ for all nine forecasters on S\&P~500 around the COVID-19 shock (shaded). The grey fill shows annualised realised volatility. The correction responds with a model-dependent lag of 77--161~days, consistent with the $w/2$ delay predicted by the trailing-window design. Chronos-Small (dark red, top) operates in the replacement regime throughout.}
	...
	four alternatives: (i)~\emph{scale correction},
	$\widehat{\VaR}_t^{\mathrm{sc}} = c \cdot
	...
	$\alpha$ \citep[cf.][]{romano2019conformalized}; and
	(iv)~\emph{isotonic regression}, a monotone mapping of
	violation indicators on $\hat{q}_t^{lo}$, which fails to
	produce a scalar VaR for most pairs (hence missing entries).
	Conformal correction achieves the lowest QS
	(5.24 $\times 10^{-4}$) and highest Green rate (86.6\%),
	followed by quantile regression on residuals (5.31, 82.4\%).
	More flexible approaches perform worse, highlighting that at
	extreme quantiles the scalar shift achieves a favourable
	bias--variance trade-off.

	The optimality of the scalar correction at extreme quantiles
	reflects a fundamental sample-size constraint. At
	$\alpha = 0.01$ with approximately 1{,}500 test observations
	per asset, the expected number of tail violations is
	${\sim}15$. Any calibration method with more than one free
	parameter must estimate its additional degrees of freedom from
	these ${\sim}15$ data points, leading to high variance that
	offsets any bias reduction. The conformal shift, with exactly
	one parameter estimated from the full calibration sample rather
	than from tail observations alone, achieves the most favourable
	bias--variance trade-off in this sparse-data regime. This
	explains the empirical ordering in
	Table~\ref{tab:baselines}: more flexible methods (isotonic
	regression, scale correction) perform strictly worse, while the
	single-parameter conformal shift and quantile regression on
	residuals---which also reduces to a near-scalar correction at
	extreme quantiles---cluster at the top.

	The comparison extends to purpose-built VaR models. GARCH-N with
	conformal correction (Table~\ref{tab:master}, Panel~A:
	QS $= 4.7 \times 10^{-4}$, 21/24 Green) dominates both EVT-POT
	(QS $= 5.5$, 18/24 Green) and filtered historical simulation
	(QS $= 5.6$, 13/24 Green), despite requiring no tail
	distribution fitting, no threshold selection, and no GPD
	estimation. A simple base model with a one-parameter conformal
	shift outperforms dedicated tail risk approaches that involve
	substantially greater implementation complexity.
	This result reflects the short 250-day rolling window and associated estimation noise in parametric tail models; under longer estimation samples or carefully tuned thresholds, EVT-based methods may perform differently.
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}lrrrrr@{}}
			...
			Conformal (ours)       & .011 &  60/216 &  5.24 & .039 & 86.6 \\
	...
	EVT Peaks-over-Threshold \citep{mcneil2000estimation} and
	filtered historical simulation
	\citep{baroneadesi1999var} are purpose-built tail risk models,
	not post-hoc calibration methods. Their inclusion in
	Table~\ref{tab:baselines} demonstrates that conformal
	correction of a simple base model (e.g., GARCH-N + conformal)
	achieves competitive or superior performance relative to more
	complex dedicated VaR approaches, while requiring substantially
	less implementation effort and no distributional assumptions.
	The EVT-POT implementation exhibited frequent fallbacks to
	empirical quantiles (GPD convergence failure rate ${>}10\%$ on
	all 24 assets), underscoring the fragility of parametric tail
	estimation on short rolling windows---a practical limitation
	that the distribution-free conformal approach avoids by design.
	...
	The Basel III Fundamental Review of the Trading Book (FRTB)
	replaces VaR with Expected Shortfall as the primary regulatory
	risk measure, making distribution-free ES calibration an urgent
	practical need. The joint score $s^J_t$ (Appendix~\ref{app:es})
	offers a pathway, but two obstacles remain: (i)~ES is not
	elicitable in isolation \citep{gneiting2011making}, requiring
	joint treatment with VaR; and (ii)~the conditioning mismatch
	(Definition~\ref{def:es_score}) means that the conformal
	correction for ES evaluated on corrected VaR violation days is
	inherently conservative. A formally guaranteed ES
	recalibration---potentially combining the Fissler--Ziegel joint
	scoring function with a two-dimensional conformal
	procedure---is the most pressing open problem for regulatory
	applications of this framework. Three technical pathways merit
	investigation: (i)~Conformal Risk Control
	\citep{angelopoulos2024conformal}, which bounds expected losses
	rather than quantile coverage, but requires extension to
	dependent data; (ii)~a joint nonconformity score based on the
	Fissler--Ziegel loss (Definition~\ref{def:joint_score}), which
	respects the joint elicitability of $(\VaR, \ES)$ but inherits
	the conditioning mismatch documented in
	Appendix~\ref{app:es}; and (iii)~tail-shape properification via
	Extreme Value Theory, which corrects distributional shape rather
	than location but introduces additional estimation uncertainty
	in the sparse tail region. Each pathway resolves one obstacle
	while introducing another; a unified solution remains open. The
	empirical results in Table~\ref{tab:es_correction} provide a
	starting point for evaluating these alternatives.
	The framework applies to univariate VaR by
	construction; portfolio-level and illiquid-asset extensions
	raise open theoretical questions under dependence.
	...
	This paper develops a one-sided conformal recalibration
	framework for Value-at-Risk that delivers approximate coverage
	guarantees under $\beta$-mixing dependence. Across five time
	series foundation models, four parametric benchmarks, and 24
	assets over 2000--2026, conformal correction achieves Basel
	Green Zone compliance for the large majority of model--asset
	combinations, with no Red Zone classifications.
	
	The conformal threshold $\qV$ serves a dual role: as a
	model-agnostic corrective shift restoring coverage, and as a
	signed diagnostic that ranks forecasters by tail adequacy and
	tracks regime-dependent miscalibration. At the same time, the
	method enforces violation frequency without improving the
	underlying predictive distribution---it is a calibration layer,
	not a substitute for model quality.
	
	Several directions remain open: extending distribution-free
	calibration to Expected Shortfall, developing formal guarantees
	for rolling implementations under dependence, and integrating
	conformal recalibration with direct quantile estimators or
	fine-tuned foundation models. A boundary condition deserves
	emphasis: the $\beta$-mixing assumption underlying the coverage
	guarantee is most strained for assets with inventory-driven
	jump dynamics (notably WTI and NATGAS), where the framework
	achieves lower Green rates despite adequate performance on
	assets with comparable or higher kurtosis.
	The underlying principle is simple: calibration can be
	separated from prediction, placing heterogeneous models on a
	common, statistically valid footing.
	
	\section*{Data and Code Availability}
	The full replication pipeline---from data download through TSFM inference, conformal calibration, backtesting, and figure generation---is available at the Quantlet \quantlet{https://github.com/QuantLet/Conformal_Oracle/}  at \url{https://github.com/QuantLet/Conformal_Oracle}. The repository includes all Monte Carlo simulation code (Section~\ref{sec:simulation}) in addition to the empirical pipeline. All return data are sourced from Yahoo Finance (public). All foundation model checkpoints are open-weight and hosted on HuggingFace (Table~\ref{tab:models}); no proprietary API access is required.
	...
	Full tables reporting $\qV$, $\hat\pi$, Traffic Light, Kupiec $p$, Christoffersen $p$, and Quantile Score for all model--asset combinations are available at \url{https://github.com/QuantLet/Conformal_Oracle}.
